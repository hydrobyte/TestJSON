...
{$IFEND >=2010}
  ...
  /// </item>
  /// <item>
  /// <term>jesEscapeSlashes</term><description>编码时转义反斜线（部分特殊场景兼容）</description>
  /// </item>
  /// <item>
  /// <term>jes1Element1Line</term><description>编码时每个数组元素单元占一行</description>
  /// </item>
  /// </list>
...
{$IFEND >=2010}
    ...
    /// <summary>编码为字节数组</summary>
    /// <param name="AEncoding">内容编码</param>
    /// <param name="ASettings">格式化设置</param>
    /// <param name="AIndent">格式化时缩进，默认为两个空格</param>
...
{$IFEND >=2010}
    ...
    /// <param name="ADoFormat">是否格式化内容</param>
    ...
    /// <param name="AEncoding">编码格式</param>
    /// <param name="AWriteBom">是否写入BOM</param>
    /// <param name="ASettings">编码参数</param>
    /// <remarks>注意当前结点的名称不会被写入</remarks>
...
{$IFEND}
    ...
    /// <summary>日期、时间、浮点格式设置，对于日期时间类型，如果不单独设置，则共用全局设置</summary>
  ...
  begin
    ...
    case p^ of
      ...
      begin
        ...
        else if (p^ <= #$7E) or (not(jesDoEscape in ASettings)) then // 英文字符区
        begin
          pd^ := p^;
          Inc(pd);
        end
